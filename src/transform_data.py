...
def normalize_datetime_columns(df: pd.DataFrame, columns_names:list[str]) -> pd.DataFrame:
    logging.info(f"\n Normalizando colunas de data e hora: {columns_names}...")
    for name in columns_names:
        df[name] = pd.to_datetime(df[name], unit='s', utc=True).dt.tz_convert('America/Sao_Paulo')
    logging.info(f"\n Colunas de data e hora normalizadas")
    return df
...
def data_transformations():
    ...
    logging.info(f"\n Transformações concluídas - DataFrame final com {len(df.columns)} colunas")
    return df
